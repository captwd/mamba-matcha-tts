...
5. **系数 × 极低 NFE 曲线**（ConvNeXt+衰减 ep134 与官方预训练）：`steps ∈ {1,2,3,4,6,10}` × `sway ∈ {关,−0.5,−1.0}`。**MCD 随步数单调下降直到 1 步最低**（48.48 / 24.18），sway 排序恒为 −1.0 > −0.5 > 关；逐句配对全部 p<0.0001。带 ASR 验证后：**`2 步 + sway(−1.0)` 综合最优**（MCD 49.03 / WER 8.7%；官方 24.58 / 7.0%，已贴近 GT-WER 上限 7.6%），比 10 步**快 5×**；1 步 MCD 最低但 WER 略逊（9.0% / 7.4%）。

**详细记录见 [`2026-09-12_summary.md`](2026-09-12_summary.md)。**
下一步：若确定改用 2 步 + sway，需按新协议重评统一评估表；可选：训练侧 sway 采 t（需重训）、cli 加 `--sway_sampling_coef`。
