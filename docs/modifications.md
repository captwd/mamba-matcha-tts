...
| `wsl_env/sway_grid.sh`、`sway_compare.py`、`sway_multi.sh`、`sway_multi_compare.py`、`sway_curve.sh`、`sway_curve_compare.py`、`sway_curve_asr.sh`、`sway_1step_asr.sh`、`synth_steps_ab.py`、`synth_steps_ab_official.py`、`check_audio_health.py`、`inspect_ckpts.py` | 新增 | A/B 网格、配对检验、多模型通用性验证、sway 系数 × 极低 NFE 曲线、WER 验证、试听音频生成（自训/官方权重）、音频能量体检、checkpoint 训练量检查脚本 |
